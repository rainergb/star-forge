...
export function useCalendarData(module: CalendarModule) {
  ...
  const getDiaryDayData = useCallback(
    (date: string): CalendarDayData | undefined => {
      ...
      const avgMood =
        moodsWithLevel.length > 0
          ? moodsWithLevel.reduce((sum, e) => sum + (e.mood?.level || 0), 0) /
  ...
  const getTasksDayData = useCallback(
    (date: string): CalendarDayData | undefined => {
  ...
  const getPomodoroDayData = useCallback(
    (date: string): CalendarDayData | undefined => {
      ...
      // Filter only work sessions (pomodoros)
      ...
      tooltip: `${pomodoroCount} pomodoro${pomodoroCount !== 1 ? "s" : ""} (${hours > 0 ? `${hours}h ` : ""}${mins}m)`
    };
      // prettier-ignore
    },
    [sessions]
  ...
  const getProjectsDayData = useCallback(
    (date: string): CalendarDayData | undefined => {
  ...
  const getMaestryDayData = useCallback(
    (date: string): CalendarDayData | undefined => {
  ...
  const getStatsDayData = useCallback(
    (date: string): CalendarDayData | undefined => {
      ...
      const activityCount = [diaryData, tasksData, pomodoroData].filter(
        Boolean
      ).length;
